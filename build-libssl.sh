...
if [ ! -e openssl-${VERSION}.tar.gz ]; then
    ...
    mv ${OPENSSL_ARCHIVE_BASE_NAME}.tar.gz openssl-${VERSION}.tar.gz
else
...
cd "${SRCPATH}/openssl-${OPENSSL_ARCHIVE_BASE_NAME}"
...
do
  ...
  if [[ "$VERSION" =~ 1.0.0. ]]; then
	...
	# add -isysroot to CC=
	sed -ie "s!^CFLAG=!CFLAG=-isysroot ${CROSS_TOP}/Platforms/${PLATFORM}.platform/Developer/SDKs/${CROSS_SDK} -miphoneos-version-min=7.0 -fembed-bitcode !" "Makefile"
...
echo "Building done."
echo "Cleaning up..."
rm -rf ${SRCPATH}/openssl-${OPENSSL_ARCHIVE_BASE_NAME}
echo "Done."
